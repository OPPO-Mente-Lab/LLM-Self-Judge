...
class MultiModalDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        list_data_dict = datasets.Dataset.from_parquet(self.data_path)
        ...
        def add_prompt(example):
            ...
            q = (example.get("extra_info") or {}).get("question")
            template = prompt_template_dict[self.prompt_type]
            problem = template.replace("{Question}", q)
            example["prompt"] = [
    ...
    def __getitem__(self, item):
        ...
        if self.image_key in row_dict:  # expand image token
            ...
            try:
                ...
                row_dict['multi_modal_data'] = {'image': [process_image(images[0], self.max_pixels, self.min_pixels, self.tool_call, self.use_tgt_size, tgt_w, tgt_h, use_tool)]}
        ...
        if self.general_qa_reward_fn in ["general_qa_gpt", "general_qa_tool", "general_qa_verifier"]:
            ...
            row_dict['ground_truth'] = row_dict.pop('original_answer', row_dict.get('reward_model', {}).get('ground_truth'))
